	...
	while (1)
	{
		...
		if (P == NULL)
		{
			...
			if (head->next == NULL)
			{
				...
				head->next->next = NULL;
				head->next->Size = 0;//если не нашлось матрицы в списке, то  size= 0 ,запомниться матрица которую ты считаешь в данный момент
			}
			else
			{
